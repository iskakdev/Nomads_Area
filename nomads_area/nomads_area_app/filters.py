...
class TourFilter(django_filters.FilterSet):
    TOUR_TYPE_CHOICES = (("group", "Групповой"), ("private", "Приватный"))
    SEASON_CHOICES = (("all_year", "Круглый год"), ("warm", "Тёплый"), ("winter", "Зима"))
    ...
    tour_type = django_filters.ChoiceFilter(field_name="tour_type", choices=TOUR_TYPE_CHOICES)
    difficulty = django_filters.ChoiceFilter(field_name="difficulty", choices=DIFFICULTY_CHOICES)
    season = django_filters.ChoiceFilter(field_name="season", choices=SEASON_CHOICES)
    ...
    def filter_by_date_from(self, qs, name, value):
        return qs.filter(
            Q(tour_type="private") | Q(tour_type="group", dates__start_date__gte=value, dates__available_spots__gte=1)
        ).distinct()

    def filter_exclude_sold_out(self, qs, name, value):
        if not value: return qs
        today = timezone.now().date()
        return qs.filter(
            Q(tour_type="private") | Q(tour_type="group", dates__start_date__gte=today, dates__available_spots__gte=1)
        ).distinct()
