...
16. [FAQ по турам](#16-faq-по-турам)
...
| Видео о компании | Ссылка на YouTube |
...
| Статус | Значение |
|--------|----------|
| **Ожидает** | Новая заявка, оплата не поступила |
| **Подтверждён** | Оплата получена, место зарезервировано |
| **Отменён** | Клиент или менеджер отменил |
| **Отклонён** | Нет мест или ошибка |

> Отменить **подтверждённое и оплаченное** бронирование через интерфейс нельзя. Обратитесь к разработчику.

---

### Как подтвердить бронирование вручную

Когда клиент оплатил наличными или переводом:

1. Откройте бронирование
2. В блоке **Платежи** найдите связанный платёж
3. Откройте платёж → выберите действие **Подтвердить оплату вручную**
4. Статус брони изменится на **Подтверждён**, места спишутся автоматически
...
Раздел: **Бронирования → Платежи**

### Статусы

| Статус | Значение |
|--------|----------|
| **Ожидает оплаты** | Запись создана, деньги не поступили |
...
Бронирование подтвердится и места спишутся автоматически.

---

## 9. Трансферы

Раздел: **Трансферы → Заявки на трансфер**

### Статусы

| Статус | Значение |
|--------|----------|
| **Ожидает** | Новая заявка |
| **Подтверждён** | Водитель назначен |
| **Отменён** | Заявка отменена |

---

### Настройка маршрутов и автомобилей

Раздел: **Трансферы → Маршруты трансфера**

1. Добавьте маршрут (Откуда → Куда)
2. В блоке **Типы машин** добавьте транспорт:
...
| Мест для багажа | Количество чемоданов |
...
| TripAdvisor | Официальный (бесплатный) | Вставить embed-код |
...
1. Войдите на [tripadvisor.com](https://tripadvisor.com)
2. Перейдите в управление бизнесом → виджеты
3. Скопируйте **embed-код**
4. В Admin: **Настройки сайта → Виджеты отзывов → TripAdvisor Widget Code**
...
Чтобы сайт работал на обоих языках — заполняйте обе вкладки.
...
> Если английский перевод не заполнен — поле будет пустым на английской версии сайта.
...
Подтвердите вручную: Платежи → выберите платёж → **Подтвердить оплату вручную**.
...
- Если **не оплачено**: Бронирования → действие **Отменить выбранные брони**
- Если **уже оплачено**: обратитесь к разработчику. Возврат денег — вручную.
...
2. Если включён — проверьте правильность App ID (Google) или embed-кода (TripAdvisor)
