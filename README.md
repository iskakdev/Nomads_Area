...
- Интеграция с платёжным провайдером FinikPay через webhook
...
- HTTP-запрос к провайдеру строго вне транзакции БД
...
```bash
git clone https://github.com/iskakdev/Nomads-Area.git
cd Nomads-Area
python -m venv .venv
...
Пользователь -> POST /bookings/
  ...
  [/transaction.atomic]
       |
  HTTP: ссылка на оплату от FinikPay (вне транзакции)
       |
...
| `FINIKPAY_API_KEY` | API ключ FinikPay |
| `FINIKPAY_WEBHOOK_SECRET` | Секрет для верификации webhook |
| `FINIKPAY_RETURN_URL` | URL после успешной оплаты |
...
Полное руководство по работе с административной панелью: [MANAGER_GUIDE.md](MANAGER_GUIDE.md)